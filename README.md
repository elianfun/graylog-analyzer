...
建議使用 Windows 工作排程器取代 Claude Routines，排程建立後不需要 Claude 開著也能自動執行：

| 任務名稱 | 排程 | 腳本 |
|---------|------|------|
| `\Graylog\Graylog-FetchHour` | 每小時整點 | `run_fetch_hour.ps1` |
| `\Graylog\Graylog-DailyReport` | 每天早上 08:00 | `run_daily_report.ps1` |
...
```bash
# 轉換指定報告
python generate_pdf.py reports/report_20260617_155649.txt
...
PDF 樣式包含：
- 深藍色頁首（報告標題、產生時間、分析引擎）
- 統計卡片（CRITICAL / ERROR / WARNING / INFO / 受影響設備數）
- 每台設備區塊（左側色條標示嚴重等級、badge、問題/原因/建議）
- 依嚴重等級自動排序（CRITICAL 優先）
...
├── generate_pdf.py          # 將 .txt 報告轉換為精美 PDF
